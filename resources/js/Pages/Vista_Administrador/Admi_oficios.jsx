...
function Admin_oficios() {
    ...
    const espacioAgrupado = useMemo(() => {
        ...
        const agrupado = espacioFiltrado.reduce((acc, item) => {
            ...
            if (!acc[mesKey].registros[item.clave_eca]) {
                acc[mesKey].registros[item.clave_eca] = {
                    ...item,
                ...
                if (platicas) {
                    acc[mesKey].registros[item.clave_eca].actividades.totalEscolar = platicas.total_escolar;
                    acc[mesKey].registros[item.clave_eca].actividades.totalComunitaria = platicas.total_comunitarias;
                }
            }

            const cantidad = parseInt(item.cantidad) || 0;
            const registroECA = acc[mesKey].registros[item.clave_eca];
